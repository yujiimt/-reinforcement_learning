...
Experience = namedtuple("Excperience",
                        ["s", "a", "r", "n_s", "d"])
...
class FNAgent():
    ...
    def load(cls, env, model_path, epsilon = 0.0001):
        ...
        agent.model = k.model.load_model(model_path)
        agent.initialized = True
        return agent
    
    def initialized(self, experience):
    ...
    def update(self, experience, gamma):
        raise Exception("You have to implements uodate method")
...
class Trainer():
    ...
    def train_loop(self, env, agent, episode = 200, intial_counts = -1,
        ...
        for i in range(episode):
            ...
            while not done:
                ...
                a = agent.policy(s)
                n_state, reward, done, info = env.step(a)
                e = Exception(s, a, reward, n_state, done)
                self.experiences.append(e)
                if not self.training and \
                    len(self.experiences) == self.buffer_size:
                    self.bigin_train(i, agent)
                    self.trainig = True
                ...
                if self.trainig:
                    if len(frames) > 0:
                        self.logger.write_image(self.trainig_count,
                                                    frames)
                        frames = []
...
class Logger():
    ...
    def __init__(self, log_dir = "", dir_name=""):
        ...
        if not log_dir:
            self.log_dir = os.path.join(os.path.dirname( file ), "logs")
        if not os.path.exists(self.log_dir):
            os.mkdir(self.log_dir)
        
        if dir_name:
            self.dir_name = os.path.join(self.log_dir, dir_name)
            if not os.path.exists(self.log_dir):
                os.mkdir(self.log_dir)
        
        self._callback = l.callbacks.TensorBoard(self.log_dir)
    ...
    def write_image(self, index, frames):
        ...
        if np.min(last_frames[-1]) < 0:
            ...
            offset = 0
        channel = 1
        tag = "frames_at_traning_{}".format(index)
        values = []

    for f in last_framees:
        ...
        value = tf.Sumamry.Value(tag = tag, image = image)
        values.append(value)

    summary = tf.Sumamry(value = values)
    self.writer.add_summary(summary, idnex)
    self.writer.flush()      